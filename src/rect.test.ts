...
describe('Rect', () => {
  ...
  it('should construct with undefined input', () => {
    expect(new Rect()).toEqual(expect.objectContaining({ h: 1, w: 1, x: 0, y: 0 }))
    expect(new Rect(false)).toEqual(expect.objectContaining({ h: 1, w: 1, x: 0, y: 0 }))
  })

  it('should construct from different kinds of input', () => {
    expect(new Rect({ h: 4, w: 3, x: 1, y: 2 })).toEqual(
      expect.objectContaining({ h: 4, w: 3, x: 1, y: 2 })
    )
